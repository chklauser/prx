...
public class StringPType : PType, ICilCompilerAware
{
    ...
    public static void Escape(string unescaped, TextWriter buffer)
    {
        for (var i = 0; i < unescaped.Length; i++)
        {
            ...
            {
                var c = unescaped[i + 1];
                nextIsHex = '0' <= c && c <= '9' || 'a' <= c && c <= 'f' || 'A' <= c && c <= 'F';
            }
            else
            {
                nextIsHex = false;
    ...
    )
    {
        ...
        switch (id.ToLowerInvariant())
        {
            ...
            case "substring":
                switch (args.Length)
                {
                    case 0:
                        return false;
                    case 1:
                        result = str[(int)args[0].ConvertTo(sctx, Int).Value!..];
                        break;
                    default:
            ...
            case "split":
                ...
                {
                    result = (PValue)_wrap_strings(str.Split(null));
                    return true;
                }
                ...
                _resolve_params(
                    sctx,
                    ((List<PValue>)list.Value!).ToArray(),
                    ref isValid,
                    sch,
                    ref sst,
                    true
    ...
    )
    {
        ...
        result = sop.Length == 0 ? String.CreatePValue("") : String.CreatePValue(sop[..^1]);
        return true;
    }
    ...
    )
    {
        ...
        throw new PrexoniteException(
            "String multiplication requires positive values. (Not " + right + ")"
        );
    }
    ...
    )
    {
        result =
            StringComparer.Ordinal.Compare(
                leftOperand.CallToString(sctx),
                rightOperand.CallToString(sctx)
            ) == 0;
        return true;
    }
