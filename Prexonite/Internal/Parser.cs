//SOURCE ARRAY
/*PTypeExpression.atg:24*/
using System;
using System.Collections.Generic;
using System.Diagnostics;
using System.IO;
using Prexonite.Types;
using FatalError = Prexonite.Compiler.FatalCompilerException; //END SOURCE ARRAY
#line 27 "F:\DotNetProjects\Prexonite\Tools\Parser.frame" //FRAME

#line default //END FRAME -->namespace

namespace Prexonite.Internal
{
#line 30 "F:\DotNetProjects\Prexonite\Tools\Parser.frame" //FRAME

    using NoDebug = DebuggerNonUserCodeAttribute;

    internal interface IScanner
    {
        Token Scan();
        Token Peek();
        void ResetPeek();

        string File
        {
            get;
        }
    }

    internal partial class Parser
    {
#line default //END FRAME -->constants

        public const int _EOF = 0;
        public const int _id = 1;
        public const int _tilde = 2;
        public const int _true = 3;
        public const int _false = 4;
        public const int _integer = 5;
        public const int _real = 6;
        public const int _string = 7;

        public enum Terminals
        {
            @EOF = 0,
            @id = 1,
            @tilde = 2,
            @true = 3,
            @false = 4,
            @integer = 5,
            @real = 6,
            @string = 7,
        }

        private const int maxT = 11;

#line 43 "F:\DotNetProjects\Prexonite\Tools\Parser.frame" //FRAME

        private const bool T = true;
        private const bool x = false;
        private const int minErrDist = 2;

        internal IScanner scanner;
        internal Errors errors;

        internal Token t; // last recognized token
        internal Token la; // lookahead token
        private int errDist = minErrDist;

#line default //END FRAME -->declarations

//SOURCE ARRAY
//END SOURCE ARRAY

#line 55 "F:\DotNetProjects\Prexonite\Tools\Parser.frame" //FRAME

        [NoDebug()]
        private Parser(IScanner scanner)
        {
            this.scanner = scanner;
            errors = new Errors();
            errors.parentParser = this;
        }

        [NoDebug()]
        private void SynErr(int n)
        {
            if (errDist >= minErrDist)
                errors.SynErr(la.line, la.col, n);
            errDist = 0;
        }

        [NoDebug()]
        internal void SemErr(string msg)
        {
            if (errDist >= minErrDist)
                errors.SemErr(t.line, t.col, msg);
            errDist = 0;
        }

        [NoDebug()]
        private void Get()
        {
            for (;;)
            {
                t = la;
                la = scanner.Scan();
                if (la.kind <= maxT)
                {
                    ++errDist;
                    break;
                }

#line default //END FRAME -->pragmas

#line 82 "F:\DotNetProjects\Prexonite\Tools\Parser.frame" //FRAME

                la = t;
            }
        }

        [NoDebug()]
        private void Expect(int n)
        {
            if (la.kind == n)
                Get();
            else
            {
                SynErr(n);
            }
        }

        [NoDebug()]
        private bool StartOf(int s)
        {
            return set[s, la.kind];
        }

        [NoDebug()]
        private void ExpectWeak(int n, int follow)
        {
            if (la.kind == n)
                Get();
            else
            {
                SynErr(n);
                while (!StartOf(follow))
                    Get();
            }
        }

        [NoDebug()]
        private bool WeakSeparator(int n, int syFol, int repFol)
        {
            bool[] s = new bool[maxT + 1];
            if (la.kind == n)
            {
                Get();
                return true;
            }
            else if (StartOf(repFol))
                return false;
            else
            {
                for (int i = 0; i <= maxT; i++)
                {
                    s[i] = set[syFol, i] || set[repFol, i] || set[0, i];
                }
                SynErr(n);
                while (!s[la.kind])
                    Get();
                return StartOf(syFol);
            }
        }

#line default //END FRAME -->productions

        private void PTypeExpression()
        {
            Type( /*PTypeExpression.atg:101*/out _lastType);
        }

        private void Type( /*PTypeExpression.atg:135*/ out PType value)
        {
            /*PTypeExpression.atg:135*/
            string id;
            List<PValue> args = new List<PValue>();

            if (la.kind == _tilde)
            {
                Get();
            }
            Expect(_id);
            /*PTypeExpression.atg:140*/
            id = t.val;
            if (!_sctx.ParentEngine.PTypeRegistry.Contains(id))
            {
                SemErr("Cannot find PType " + id + " referenced in PTypeExpression.");
                id = null;
            }

            if (la.kind == 8)
            {
                Get();
                /*PTypeExpression.atg:147*/
                PValue obj;
                if (StartOf(1))
                {
                    Expr( /*PTypeExpression.atg:148*/out obj);
                    /*PTypeExpression.atg:148*/
                    args.Add(obj);
                    while (WeakSeparator(9, 1, 2))
                    {
                        Expr( /*PTypeExpression.atg:150*/out obj);
                        /*PTypeExpression.atg:150*/
                        args.Add(obj);
                    }
                }
                Expect(10);
            }
            /*PTypeExpression.atg:155*/
            if (id == null) //Error recovery.
                value = PType.Object[typeof(object)];
            else //Normal case
                value = _sctx.ConstructPType(id, args.ToArray());
        }

        private void Expr( /*PTypeExpression.atg:104*/ out PValue value)
        {
            /*PTypeExpression.atg:104*/
            PType vt;
            value = null;
            if (la.kind == _true || la.kind == _false)
            {
                Boolean( /*PTypeExpression.atg:106*/out value);
            }
            else if (la.kind == _integer)
            {
                Integer( /*PTypeExpression.atg:107*/out value);
            }
            else if (la.kind == _real)
            {
                Real( /*PTypeExpression.atg:108*/out value);
            }
            else if (la.kind == _string)
            {
                String( /*PTypeExpression.atg:109*/out value);
            }
            else if (la.kind == _id || la.kind == _tilde)
            {
                Type( /*PTypeExpression.atg:110*/out vt);
                /*PTypeExpression.atg:110*/
                value = PType.Object.CreatePValue(vt);
            }
            else
                SynErr(12);
        }

        private void Boolean( /*PTypeExpression.atg:113*/ out PValue value)
        {
            /*PTypeExpression.atg:113*/
            value = true;
            if (la.kind == _true)
            {
                Get();
            }
            else if (la.kind == _false)
            {
                Get();
                /*PTypeExpression.atg:116*/
                value = false;
            }
            else
                SynErr(13);
        }

        private void Integer( /*PTypeExpression.atg:120*/ out PValue value)
        {
            Expect(_integer);
            /*PTypeExpression.atg:121*/
            value = Int32.Parse(t.val);
        }

        private void Real( /*PTypeExpression.atg:125*/ out PValue value)
        {
            Expect(_real);
            /*PTypeExpression.atg:126*/
            value = Double.Parse(t.val);
        }

        private void String( /*PTypeExpression.atg:130*/ out PValue value)
        {
            Expect(_string);
            /*PTypeExpression.atg:131*/
            value = StringPType.Unescape(t.val.Substring(1, t.val.Length - 2));
        }

#line 121 "F:\DotNetProjects\Prexonite\Tools\Parser.frame" //FRAME

        public void Parse()
        {
            la = new Token();
            la.val = "";
            Get();

#line default //END FRAME -->parseRoot

            PTypeExpression();

#line 127 "F:\DotNetProjects\Prexonite\Tools\Parser.frame" //FRAME

            Expect(0);
        }

        private bool[,] set = {
#line default //END FRAME -->initialization

                                  {T, x, x, x, x, x, x, x, x, x, x, x, x},
                                  {x, T, T, T, T, T, T, T, x, x, x, x, x},
                                  {x, x, x, x, x, x, x, x, x, x, T, x, x}

#line 132 "F:\DotNetProjects\Prexonite\Tools\Parser.frame" //FRAME
                              };
    } // end Parser

    [NoDebug()]
    internal class Errors
    {
        internal int count = 0; // number of errors detected
        internal TextWriter errorStream = Console.Out; // error messages go to this stream

        internal string errMsgFormat = "-- ({3}) line {0} col {1}: {2}";
                        // 0=line, 1=column, 2=text, 3=file

        internal Parser parentParser;

        internal void SynErr(int line, int col, int n)
        {
            string s;
            switch (n)
            {
#line default //END FRAME -->errors

                case 0:
                    s = "EOF expected";
                    break;
                case 1:
                    s = "id expected";
                    break;
                case 2:
                    s = "tilde expected";
                    break;
                case 3:
                    s = "true expected";
                    break;
                case 4:
                    s = "false expected";
                    break;
                case 5:
                    s = "integer expected";
                    break;
                case 6:
                    s = "real expected";
                    break;
                case 7:
                    s = "string expected";
                    break;
                case 8:
                    s = "\"(\" expected";
                    break;
                case 9:
                    s = "\",\" expected";
                    break;
                case 10:
                    s = "\")\" expected";
                    break;
                case 11:
                    s = "??? expected";
                    break;
                case 12:
                    s = "invalid Expr";
                    break;
                case 13:
                    s = "invalid Boolean";
                    break;

#line 146 "F:\DotNetProjects\Prexonite\Tools\Parser.frame" //FRAME

                default:
                    s = "error " + n;
                    break;
            }
            if (s.EndsWith(" expected"))
                s += " and not \"" + parentParser.t.ToString(false) + " " +
                     parentParser.la.ToString(false) + "\"";
            else if (s.StartsWith("this symbol "))
                s = "\"" + parentParser.t.val + "\"" + s.Substring(12);
            errorStream.WriteLine(errMsgFormat, line, col, s, parentParser.scanner.File);
            count++;
        }

        internal void SemErr(int line, int col, string s)
        {
            errorStream.WriteLine(errMsgFormat, line, col, s, parentParser.scanner.File);
            count++;
        }

        internal void SemErr(string s)
        {
            errorStream.WriteLine(s);
            count++;
        }

        internal void Warning(int line, int col, string s)
        {
            errorStream.WriteLine(errMsgFormat, line, col, s, parentParser.scanner.File);
        }

        internal void Warning(string s)
        {
            errorStream.WriteLine(s);
        }
    } // Errors

#line default //END FRAME $$$
}