...
namespace Prx.Tests
{
    ...
    public class TypeSystem
    {
        ...
        public void TestFieldAccess()
        {
            ...
            //Set obj.Subject = 55
            obj.DynamicCall(sctx,
                            new PValue[] {55}, PCall.Set, "Subject");
            Assert.AreSame(test, obj.Value);
            Assert.AreEqual(test.Subject, ((TestObject) obj.Value).Subject);

            //Get res = obj.Subject
            PValue res = obj.DynamicCall(sctx,
                                         new PValue[] {}, PCall.Get, "Subject");
            Assert.AreEqual(test.Subject, res.Value);

            //Set obj.Count = res
            obj.DynamicCall(sctx,
                            new PValue[] {res}, PCall.Set, "Count");
            Assert.AreEqual(55, test.Count);

            //Get res = obj.Count
            res = obj.DynamicCall(sctx,
                                  new PValue[] {}, PCall.Get, "Count");
            Assert.AreEqual(55, test.Count);
        ...
        {
            PType res = sctx.ConstructPType("Object", new PValue[] { PType.Object.CreatePValue(typeof(Int32))});
        ...
        {
            PType res = sctx.ConstructPType(typeof(ObjectPType), new PValue[] { PType.Object.CreatePValue(typeof(DateTime))});
        ...
        {
            PType res = sctx.ConstructPType(PType.Object[typeof(ObjectPType)], new PValue[] { PType.Object.CreatePValue(typeof(Thread))});
        ...
        {
            string sEscaped = @"This is a \n followed by a \ttab and an umlaut: \x" + ((int) 'ä').ToString("X") +
                              " escape sequence.";
            ...
            PValue unescaped;
            Assert.IsTrue(escaped.TryDynamicCall(sctx, new PValue[] {}, PCall.Get, "unescape", out unescaped));
            Assert.AreEqual(sUnescaped, unescaped.Value as string);
            Assert.IsTrue(unescaped.TryDynamicCall(sctx, new PValue[] {}, PCall.Get, "escape", out escaped));
