...
public abstract class Translation : VMTestsBase
{
    ...
    public void TrailingCommaHashLiteral()
    {
        ...
        Expect(
            "110101",
            (PValue)new List<PValue> { 1, 2, 4, 3, 2, 1 },
    ...
    public void AppendRightLocalFunc()
    {
        ...
        Expect(
            " 1 2 3 4 5 6 7[ 33: 1, 33: 2, 77: 1, 77: 2, 77: 3, 77: 4, 77: 5, 77: 6, 77: 7 ]",
            1,
            2,
            3,
            4,
    ...
    public void TestPsrAst3WithPos()
    {
        ...
        var astPlaceholder = new AstPlaceholder(
            ...
            NoSourcePosition.Instance.Line,
            NoSourcePosition.Instance.Column
        );
        factory.Setup(f => f.Placeholder(It.IsAny<ISourcePosition>(), 5)).Returns(astPlaceholder);
        ExpectNamed(
            "ast3\\withPos",
            astPlaceholder,
            sctx.CreateNativePValue(factory.Object),
            "Placeholder",
            sctx.CreateNativePValue(NoSourcePosition.Instance),
            5
        );
    }
    ...
    public void EntityRefToCommand()
    {
        ...
        Expect(rv =>
        {
            ...
            Assert.That(
                (object)r.Length,
                Is.EqualTo(8),
                "Expected return value to consist of 8 elements. Returned {0}",
                rv
            );
            ...
            Assert.That(
                r[2],
                Is.EqualTo(EntityRef.Variable.Global.Create(nameof(r), nm).ToString())
            );
    ...
    public void NamespaceDeclaration()
    {
        ...
        Expect(17);
        Assert.That(
            ldr.TopLevelSymbols.TryGet("f", out var dummy),
            Is.False,
            "Existence of symbol f in the global scope"
        );
    }
    ...
    public void NamespaceExtensionCrossModule()
    {
        ...
        var moduleTwoDesc = plan.Assemble(
            Source.FromString(
                """
        ...
        Assert.That(
            moduleTwo.Messages.Where(m => m.Severity == MessageSeverity.Error),
            Is.Empty,
            "Modules should compile without any error messages."
        );
        ...
        Assert.That(
            symbols.TryGet("a", out var symbol),
            Is.True,
            "Expect module two to have a symbol called 'a'"
        );
        Assert.That(symbol, Is.InstanceOf<NamespaceSymbol>());
        var nsSym = (NamespaceSymbol?)symbol;
        Assert.That(nsSym, Is.Not.Null);
        Assert.That(
            nsSym!.Namespace.TryGet("b", out symbol),
            "Expect namespace a to contain a symbol b."
        );
        Assert.That(
            nsSym.Namespace.TryGet("c", out symbol),
            "Expect namespace a to contain a symbol c."
        );
    }
    ...
    public void DotSeparatedMetaValues()
    {
        ...
        Assert.That(
            f.Meta["key3"],
            Is.EqualTo(new MetaEntry([new MetaEntry("a.b"), new MetaEntry("c")]))
        );
    }
    ...
    public void ErrorDoubleColonInNamespaceDecl()
    {
        ...
        Assert.That(
            ldr.Errors.Where(m =>
                m.MessageClass == MessageClasses.UnexpectedDoubleColonInNamespaceName
            ),
            Is.Not.Empty
        );
    ...
    public void WarningUnqualifiedNamespace()
    {
        ...
        Assert.That(
            ldr.Warnings.Where(m => m.MessageClass == MessageClasses.UnqualifiedImport),
            Is.Not.Empty
        );
    ...
    public void ErrorUnresolvedNamespace()
    {
        ...
        Assert.That(
            ldr.Errors.Where(m => m.MessageClass == MessageClasses.NamespaceExcepted),
            Is.Not.Empty
        );
    ...
    public void CustomTypeFunctionsInNamespaces()
    {
        ...
        Expect(new List<PValue> { "pot(X)", "Y~pot", "pot.heat(X)", true, true, true }, "X", "Y");
    }
    ...
    public void CustomTypeFunctionsWithDotSuffix()
    {
        ...
        Expect(new List<PValue> { "pot()", "Y~pot", "pot.heat", "True", "True", "True" }, "Y");
    }
    ...
    public void QuestionMarkSpliceIsInvalid()
    {
        ...
        Assert.That(
            ldr.Errors.Where(m => m.MessageClass == MessageClasses.IncompleteBinaryOperation),
            Is.Not.Empty
        );
    }
