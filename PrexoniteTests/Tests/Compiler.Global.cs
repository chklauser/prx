...
public class CompilerTestBaseGlobal : CompilerTestBase
{
    ...
    {
        const string input1 = "MyList { elem1, elem2, elem3 };";
        var opt = new LoaderOptions(engine, target);
        opt.UseIndicesLocally = false;
        var ldr = new Loader(opt);
        ldr.LoadFromString(input1);
        ...
        const string input2 = "Add elem4 to MyList;";
        ...
        Assert.IsTrue(
            target.Meta["MyList"].IsList,
            "MyList should be a list, even after modification"
        );
        Assert.AreEqual(4, target.Meta["MyList"].List.Length);
        Assert.AreEqual("elem4", target.Meta["MyList"].List[3].Text);

        const string input3 = "Add { elem5, elem6 } to MyList;";
        ...
        Assert.IsTrue(
            target.Meta["MyList"].IsList,
            "MyList should be a list, even after 2nd modification"
        );
        ...
        Assert.IsTrue(
            target.Meta["Import"].IsList,
            "Import should be a list after 2nd modification"
        );
    ...
    public void Declare()
    {
        ...
        Assert.AreEqual(
            new SymbolEntry(SymbolInterpretations.Function, "f\\1", target.Module.Name),
            LookupSymbolEntry(symbols, @"f\1")
        );
        Assert.AreEqual(
            new SymbolEntry(
        ...
        Assert.AreEqual(
            new SymbolEntry(SymbolInterpretations.Function, "if\\1", target.Module.Name),
            LookupSymbolEntry(symbols, @"if\1")
        );
    }
    ...
    public void Redeclare()
    {
        ...
        Assert.AreEqual(
            new SymbolEntry(SymbolInterpretations.Function, "name1", target.Module.Name),
        ...
        const string input2 = "declare function name1; " + "declare function name2; ";
        ...
        Assert.AreEqual(
            new SymbolEntry(SymbolInterpretations.Function, "name2", target.Module.Name),
            LookupSymbolEntry(symbols, "name2")
        );
    }
    ...
    public void DefineGlobal()
    {
        ...
        const string input1 = "declare var name1; " + "declare ref name2; " + "declare var value;";
        var opt = new LoaderOptions(engine, target);
        opt.UseIndicesLocally = false;
        var ldr = new Loader(opt);
        ldr.LoadFromString(input1);
        Assert.AreEqual(
            0,
            ldr.ErrorCount,
            "The compiler reported errors in the first chunk of code."
        );
        var symbols = ldr.Symbols;
        ...
        //Then define them
        const string input2 =
            "var name1; " + "ref name2 [ Description NotUseful; ]; " + "var name3;";

        ldr.LoadFromString(input2);
        Assert.AreEqual(
            0,
            ldr.ErrorCount,
            "The compiler reported errors in the second chunk of code."
        );
        Assert.AreEqual(
            new SymbolEntry(
                SymbolInterpretations.GlobalObjectVariable,
    ...
    public void InterpreterLineAtBeginning()
    {
        ...
        Assert.That(
            ldr.ParentApplication.Meta[Application.InterpreterLineKey],
            Is.EqualTo(new MetaEntry("/usr/bin/env prx --legacy"))
        );
        Assert.That(
            ldr.ParentApplication.Meta["SomeOtherSettings"],
    ...
    public void InterpreterLineIncludedInStoredRepresentation()
    {
        ...
        Assert.That(
            loader2.ParentApplication.Meta,
            Does.ContainKey(Application.InterpreterLineKey)
        );
        Assert.That(loader2.ParentApplication.Meta, Does.ContainKey("SomeOtherSettings"));

        Assert.That(
            loader2.ParentApplication.Meta[Application.InterpreterLineKey],
            Is.EqualTo(new MetaEntry(previousInterpreterLine))
        );
        Assert.That(
            loader2.ParentApplication.Meta["SomeOtherSettings"],
            Is.EqualTo(new MetaEntry("Are Valid"))
        );
    }
    ...
    public void EmptyAsmFunction()
    {
        ...
        Assert.AreEqual(
            ...
            LookupSymbolEntry(ldr.Symbols, "func1").Interpretation
        );
        ...
        Assert.AreEqual(
            SymbolInterpretations.Function,
            LookupSymbolEntry(ldr.Symbols, "func2").Interpretation
        );
        ...
        Assert.AreEqual(
            SymbolInterpretations.Function,
            LookupSymbolEntry(ldr.Symbols, "func3").Interpretation
        );
    ...
    public void GlobalVariableShadowId()
    {
        ...
        Assert.IsTrue(
            a.Interpretation == SymbolInterpretations.GlobalObjectVariable,
        ...
        Assert.IsTrue(
            b.Interpretation == SymbolInterpretations.GlobalObjectVariable,
            "Symbol b must be global object variable."
        );
        Assert.IsTrue(
            target.Variables.ContainsKey(b.InternalId!),
        ...
        Assert.IsTrue(
            c.Interpretation == SymbolInterpretations.GlobalObjectVariable,
            "Symbol c must be global object variable."
        );
        Assert.IsTrue(
            target.Variables.ContainsKey(c.InternalId!),
            "Symbol c must point to a physical variable."
        );
        Assert.IsTrue(
            b.InternalId == c.InternalId,
            "Symbols b and c must point to the same variable."
        );

        Assert.IsTrue(ldr.Symbols.Contains("d"), "Symbol d must exist.");
        var d = LookupSymbolEntry(ldr.Symbols, "d");
        Assert.IsTrue(
            d.Interpretation == SymbolInterpretations.GlobalObjectVariable,
        ...
        Assert.IsTrue(
            e.Interpretation == SymbolInterpretations.GlobalObjectVariable,
            "Symbol e must be global object variable."
        );
        Assert.IsTrue(
            target.Variables.ContainsKey(e.InternalId!),
        ...
        Assert.IsTrue(
            f.Interpretation == SymbolInterpretations.GlobalObjectVariable,
            "Symbol f must be global object variable."
        );
        Assert.IsTrue(
            target.Variables.ContainsKey(f.InternalId!),
            "Symbol f must point to a physical variable."
        );
        Assert.IsTrue(
            e.InternalId == f.InternalId,
            "Symbols e and f must point to the same variable."
        );
