...
namespace CSFlex
{
    ...
    public sealed class DFA
    {
         ...
         */
        private Hashtable usedActions = new PrettyHashtable();
        ...
        public DFA(int numLexStates, int numInp)
        {
            ...
            log.AutoFlush = true;
            log.WriteLine(
                "new DFA(int numLexStates = {0}, int numInp = {1})",
                numLexStates,
                numInp
            );
#endif // DEBUG_TRACE
...
#if DEBUG_TRACE
            log.WriteLine(
                "setAction(int state = {0}, Action stateAction = {1})",
                state,
                stateAction
            );
#endif // DEBUG_TRACE
        ...
        public void checkActions(LexScan scanner, LexParse parser)
        {
            ...
            while (l.MoveNext())
            {
                ...
                if (!next.Equals(usedActions[next]) && !eofActions.isEOFAction(next))
                    Out.warning(
                        scanner.file,
                        ErrorMessages.NEVER_MATCH,
                        ((Action)next).priority - 1,
                        -1
                    );
            }
        }
            ...
            for (int i = n; i <= last; i++)
            {
                ...
                while (s != i)
                {
                    ...
                    if (s != i)
                    {
                        ...
                        if (b[s] != i)
                            Out.dump(
                                "consistency error for state " + (s - 1) + " (block " + b[s] + ")"
                            );
                    }
                    if (b_b[s] != t)
                        Out.dump(
