...
// Modified in July 2026 by the Prexonite contributors to load the embedded
// message catalogue without the .NET Framework ResX API. See COPYRIGHT.
...
public class ErrorMessages {
  ...
  static ErrorMessages()
  {
    ...
    using Stream resx_stream = assembly.GetManifestResourceStream("csflex.Messages.xml");
    XDocument document = XDocument.Load(resx_stream);
    ...
    foreach (XElement data in document.Root.Elements("data"))
    {
      ...
      if (name != null && value != null)
        resources[name.Value] = value.Value;
    }
  }
